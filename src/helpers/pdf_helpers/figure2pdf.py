...
def _draw_page_template(
    ...
    header_ax.add_patch(
        mpatches.Rectangle((0, 0.88), 1, 0.12,
                            facecolor=COLOR_ACCENT2, edgecolor='none',
...
def _draw_task_block(
    ...
    for i, task in enumerate(tasks, 1):
        ...
        if task.get('description'):
            fig.text(
                x_left + 0.055, y - 0.028,
                task['description'],
                fontsize=desc_fontsize,
                va='top', color=COLOR_MUTED,
                fontfamily='monospace',
                style='italic',
                wrap=True,
            )
...
def save_figure_pinhole(
    ...
    """
    Save figure as PDF with logo and footer text (for pinhole / camera obscura result).
    """
    if not filename.endswith('.pdf'):
        filename += '.pdf'
    ...
    tasks = [
        ...
        },
        {
            ...
            'description': (
                'Эклиптика — видимый путь Солнца среди звёзд (наклонена к экватору\n'
                'на ε ≈ 23°26′). Нанесите её, если она попадает в поле зрения снимка.'
            ),
        },
        {
        ...
        },
        {
            ...
            'description': (
                'Нанесите точки осеннего или весеннего равноденствия, если они видны.\n'
...
def save_figure_skychart(
    ...
    chart_margins: tuple[float, float, float, float] = (0.03, 0.20, 0.03, 0.135),
    ...
    tasks = [
        ...
        },
        {
            ...
            'description': (
                'Полюс мира P лежит на оси вращения Земли. Для северного полушария\n'
        ...
        },
        {
            'label': 'Проведите эклиптику',
            'description': (
                'Эклиптика — путь Солнца за год, наклонена к экватору на ε ≈ 23°26′.\n'
        ...
        },
        {
            'label': 'Проведите небесный экватор',
            'description': (
                'Небесный экватор перпендикулярен оси P–Z. В стереографической\n'
        ...
        },
        {
            ...
            'description': (
                ...
                'пересечение эклиптики с экватором. Нанесите ту, что попадает\n'
                'в поле зрения карты.'
            ),
        },
        {
            ...
            'description': (
                'Запишите результат: ' + '_ ' * 12
            ),
        },
        {
            'label': 'Определите широту наблюдателя φ',
            'description': (
                'Обратите внимание что карта дана в стереографической проекции \n'
                'Запишите ответ: ' + '_ ' * 14
            ),
        },
        {
    ...
    with PdfPages(filename) as pdf:
        ...
        if print_skychart_info:
            info_lines: list[str] = [
                ...
                f"Звёздное время   : {get_sidereal_time(longitude=longitude, local=observation_time)}",
            ]
            if latitude is not None and longitude is not None:
                loc = f'{latitude:.2f}°,  {longitude:.2f}°'
                prefix = f'Место            : {location_name}  ({loc})' if location_name else f'Координаты       : {loc}'
                info_lines.append(prefix)
            elif location_name:
                info_lines.append(f'Место            : {location_name}')

            # Light info box — use a dedicated axes so FancyBboxPatch renders correctly
            n = len(info_lines)
            box_h = 0.015 + 0.024 * n
        ...
        if square_height_frac < height:
            # We have more vertical room than needed — centre the square vertically.
