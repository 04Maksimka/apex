...
def _rk4_step(state, dt):
    ...
    x0, y0, vx0, vy0 = state
    k1 = deriv(state)
    k2 = deriv((x0 + k1[0]*dt/2, y0 + k1[1]*dt/2,
                vx0 + k1[2]*dt/2, vy0 + k1[3]*dt/2))
    k3 = deriv((x0 + k2[0]*dt/2, y0 + k2[1]*dt/2,
                vx0 + k2[2]*dt/2, vy0 + k2[3]*dt/2))
    k4 = deriv((x0 + k3[0]*dt, y0 + k3[1]*dt,
                vx0 + k3[2]*dt, vy0 + k3[3]*dt))
    return (
...
def _compute_conic(r0x, r0y, v0x, v0y, n_points=6000):
    ...
    if ecc < 1.0:
        # Равномерно по истинной аномалии, но с удвоенной плотностью у перицентра
        raw = [2 * math.pi * i / n_points for i in range(n_points + 1)]
        # Нелинейное переотображение: сжимаем θ ближе к 0 (перицентр)
        thetas = [math.copysign(abs(t) ** 0.65 * math.pi ** 0.35, t)
                  if t <= math.pi
...
def _compute_flat(r0x, r0y, v0x, v0y, n_points=400):
    ...
    for i in range(n_points + 1):
        ...
        if xv < 0:
            ...
            break
        pts.append([r0x + xh*hor_x + xv*up_x,
                    r0y + xh*hor_y + xv*up_y])
